# Hyperframes Editor — Armina's Video Studio

HTML-native video workspace built on [Hyperframes](https://hyperframes.heygen.com). Armina's personal video editing studio for the full pipeline: raw footage → filler-word removal → motion graphics → final MP4. Not a Remotion project — this is the other pipeline.
...
**`MOTION_PHILOSOPHY.md` (at the workspace root) is the canonical motion-graphics aesthetic for this workspace.** It is the deconstructed playbook of the Infinite Global Payments 30s spot — the gold standard Armina has chosen for every motion build going forward.
...
5. **Run the pre-flight checklist** before claiming any motion piece is done. The "What Would Infinite Do?" 10-question test in section 5 is a good gut check before showing Armina.

**When NOT to apply it:**
- If a brand brief explicitly demands a different aesthetic, keep the *discipline* (one idea per beat, motion in transitions, breathing outros, callbacks) but adapt the *palette and texture* to the brand spec in `DESIGN.md`.

If `MOTION_PHILOSOPHY.md` is missing from the workspace root, stop and ask Armina before brainstorming — it should always be there.
...
```
ArminaSwittenburg/
├── CLAUDE.md, AGENTS.md, DESIGN.md          ← workspace docs
├── DESIGN.ais-example.md                    ← student kit example (reference only)
├── MOTION_PHILOSOPHY.md                     ← gold-standard motion-graphics aesthetic (READ before brainstorming)
├── scripts/filler-cut.mjs                   ← filler word removal pipeline
├── package.json, node_modules/              ← workspace tooling
├── .claude/                                 ← skills + plugin config
├── assets/                                  ← shared brand assets (logo, brand-tokens.css, music)
└── video-projects/                          ← one folder per video
    ├── claude-edit-intro/                   ← reference: transcribe→edit→MG pipeline example
    ├── may-shorts-6/                        ← reference: short-form vertical video template
    └── <your-project>/                      ← Armina's videos go here
```
...
- **Motion-graphics philosophy:** `MOTION_PHILOSOPHY.md` (gold-standard aesthetic — read before brainstorming any composition)
- **Brand source-of-truth:** `DESIGN.md` (Armina's brand spec — colors, fonts, logo, motion rules). Copy `assets/brand-tokens.css` into each project's `assets/` when needed.
- **Filler word pipeline:** `scripts/filler-cut.mjs` — runs transcription + cut-list generation + FFmpeg splice
- **Reference example:** `DESIGN.ais-example.md` (kept from student kit as a worked example of how to write a DESIGN.md)
- **Raw-recording stash:** large source MP4s/MP3s not yet assigned to a project can sit in `raw-media/` (gitignored) until moved into a project's `assets/`
...
5. **Localhost Studio preview** — before **any** render (even a draft), start `npx hyperframes preview` in the background and hand Armina the URL. He eyeballs the edit live and iterates; no render cycle until he's seen it. See "Localhost Preview Before Any Render" below.
6. Only after Armina says the live preview looks right: `render --quality draft` for a draft MP4.
...
Lint passing ≠ design working. Never tell Armina a render is done until you have actually looked at the frames. No exceptions. A "successful" render with a cropped face, misaligned text, or a scene landing on the wrong word is a broken render — and lint exit codes will not catch any of that. Armina has explicitly asked for this gate — treat skipping it as a regression.
...
3. Call `Read` on every PNG so the image actually loads into context. Do NOT just list filenames. Verify:
   ...
   - Captions are on-brand (per `DESIGN.md`) and readable
   - No text overflow, no unintentional overlap, no blank frames
4. If anything is wrong — fix, re-render, re-verify. Never ship a broken render and let Armina find the bug.
...
**Every edit pass gets two preview gates**: one on the live Studio **before** any render (so Armina can iterate on cheap edits without waiting for a render), and one on the rendered MP4 **before** the final `--quality standard` bake. Do not run `render --quality draft` OR `render --quality standard` until Armina has eyeballed the relevant state in his browser.
...
3. Hand Armina the URL + tell him exactly which sub-compositions to scrub (individual comp URLs load fastest — the master composition can stall when it includes WebGL shader blocks under software WebGL fallback). Example: `http://localhost:3002/?comp=v01-kinetic-type`.
4. Wait for Armina's explicit sign-off on the live preview ("looks good, render a draft" / "ship it" / "go ahead"). Silence is not approval.
...
6. Serve `renders/` via `npx serve . -p 8080 -n` (NOT Python's `http.server` — it doesn't support HTTP Range requests, so scrubbing breaks). Hand Armina `http://localhost:8080/<project>-draft.mp4`.
...
**If the master composition stalls in Studio** (software WebGL + multiple shader blocks): route Armina to individual sub-composition URLs instead. They load instantly and isolate the change you're previewing.

The render is the ground truth. "The code looks correct" doesn't clear the bar.

## Raw Video → Edited MP4 Pipeline (Filler Word Removal)

**When Armina drops a raw video file, run this pipeline before building any compositions.**

### Step 1 — Re-encode to H.264
...
```bash
cd video-projects/<project-slug>
npx hyperframes transcribe assets/clip.mp4 --model small.en --json
# outputs: assets/clip.transcript.json
```

Or via the workspace-level OpenAI Whisper script:
```bash
node scripts/filler-cut.mjs assets/clip.mp4 --transcribe-only
```

### Step 3 — Remove filler words

```bash
node scripts/filler-cut.mjs assets/clip.mp4 --transcript assets/clip.transcript.json
# outputs: assets/clip-edited.mp4  (fillers cut, no gaps)
# outputs: assets/clip-edited.transcript.json  (timestamps remapped to edited time)
```

Default filler word list: `um, uh, like, you know, basically, literally, honestly, actually, right, so, okay`. Override with `--fillers "um,uh,hmm"`.

Review the cut log (`assets/clip-edited.cutlog.json`) before proceeding — it lists every removed segment with the word and timestamp so Armina can spot false positives.

### Step 4 — Build motion graphics on top

Use `/short-form-video` (vertical 9:16) or `/make-a-video` (any format) with `assets/clip-edited.mp4` as the source and `assets/clip-edited.transcript.json` for caption timing. All timestamps in the transcript are already in edited-time — do not mix with original timestamps.

### Step 5 — Render and verify

Follow the standard authoring loop (lint → Studio preview → draft render → visual verification → final render).
